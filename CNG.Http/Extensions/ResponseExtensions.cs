            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (!response.IsSuccessStatusCode)
                return new HttpClientErrorResponse<TResponse>(
                    exceptionHandler != null ? exceptionHandler(message, exceptionResponseType) : message,
                    response.Headers,
                    response.StatusCode);
            try
            {
            ...
            {
                return new HttpClientErrorResponse<TResponse>(
                    exceptionHandler != null ? exceptionHandler($"{message} | {ex.Message}", exceptionResponseType) : $"{message} | {ex.Message}", response.Headers,
                    response.StatusCode);
            }
