...
namespace CNG.MongoDB
{
    ...
    where TKey : IEquatable<TKey>
  {
    ...
    public MongoDbRepository(IMongoDbContext context) => _context = context;

    public virtual async Task<ICollection<TEntity>> GetAllAsync(
      Expression<Func<TEntity, bool>>? filter = null,
      CancellationToken cancellationToken = default (CancellationToken))
    {
      ICollection<TEntity> async = await FindAsync((FilterDefinition<TEntity>) filter, cancellationToken: cancellationToken);
      return async;
    }

    public virtual async Task<bool> AnyAsync(TKey id, CancellationToken cancellationToken = default (CancellationToken))
    {
      var entity = await FindOneAsync(Filter.IdEq(id), cancellationToken: cancellationToken);
      var result = entity;
    ...
    {
      var oneAsync = await FindOneAsync(Filter.IdEq(id), cancellationToken: cancellationToken);
      return oneAsync;
    }
    ...
    public virtual async Task DeleteRangeAsync(
      ...
      CancellationToken cancellationToken = default (CancellationToken))
    {
        List<Task> tasks = entities.Select((Func<TEntity, Task>) (entity => DeleteAsync(entity, cancellationToken))).ToList();
      await Task.WhenAll(tasks);
    }
